...
const COMMUNITY_BADGE_SELECTOR =
...
function applyPartnerGuildBadge(): void {
    ...
    try {
        ...
        const isPartnerGuild = Boolean(guildId && partnerGuildIds.has(guildId));
        ...
        for (const target of document.querySelectorAll<HTMLElement>(COMMUNITY_BADGE_SELECTOR)) {
            patchCommunityBadge(target, guildId);
        }
    } finally {
        applying = false;
